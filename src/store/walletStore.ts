  ...
  persist(
    (set, get) => ({
      ...
      getChainForWallet: (wallet) => {
        ...
        if (wallet.chain === "EVM")
          return EVM_CHAINS.find((c) => c.id === wallet.chainId);
        // Cosmos mapping (improved via address prefix)
        if (wallet.chain === "Cosmos") {
          if (wallet.address.startsWith("cosmos"))
            return {
              chain_id: "cosmoshub-4",
              chain_name: "cosmos",
              chain_type: "cosmos",
            };
          if (wallet.address.startsWith("juno"))
            return {
              chain_id: "juno-1",
              chain_name: "juno",
              chain_type: "cosmos",
            };
          if (wallet.address.startsWith("neutron"))
            return {
              chain_id: "neutron-1",
              chain_name: "neutron",
              chain_type: "cosmos",
            };
          if (wallet.address.startsWith("osmo"))
            return {
              chain_id: "osmosis-1",
              chain_name: "osmosis",
              chain_type: "cosmos",
            };
          if (wallet.address.startsWith("atone"))
            return {
              chain_id: "atomone-1",
              chain_name: "atomone",
              chain_type: "cosmos",
        ...
        try {
          ...
          if (chain === "Cosmos") {
            if (window.keplr) {
              ...
              const cosmosChains = [
                {
                  id: "cosmoshub-4",
                  rpcChain: "COSMOS_HUB" as const,
                  symbol: "ATOM",
                },
                { id: "juno-1", rpcChain: "JUNO" as const, symbol: "JUNO" },
                {
                  id: "neutron-1",
                  rpcChain: "NEUTRON" as const,
                  symbol: "NTRN",
                },
                {
                  id: "osmosis-1",
                  rpcChain: "OSMOSIS" as const,
                  symbol: "OSMO",
                },
                {
                  id: "atomone-1",
                  rpcChain: "ATOM_ONE" as const,
                  symbol: "ATONE",
                },
                {
                  id: "stargaze-1",
                  rpcChain: "STARGAZE" as const,
                  symbol: "STARS",
                },
              ];
              ...
              for (const chainConfig of cosmosChains) {
                try {
                  ...
                  const nativeBal = await RpcService.getBalance(
                    chainConfig.rpcChain,
                    cosmosAddress,
                  );
                  ...
                  const newWallet: ConnectedWallet = {
                    ...
                    name:
                      chainConfig.id === "cosmoshub-4"
                        ? "Cosmos Hub"
                        : chainConfig.id === "juno-1"
                          ? "Juno Network"
                          : chainConfig.id === "osmosis-1"
                            ? "Osmosis"
                            : chainConfig.id === "atomone-1"
                              ? "Atom One"
                              : "Neutron",
                    chain: "Cosmos",
                  ...
                  if (
                    !currentWallets.find(
                      (w) =>
                        w.address === cosmosAddress &&
                        w.symbol === chainConfig.symbol,
                    )
                  ) {
                    set((state) => ({
                  ...
                  if (chainConfig.id === "juno-1") {
                    ...
                    phmnBal = await RpcService.getCw20Balance(
                      "JUNO",
                      PHMN_CONTRACT,
                      cosmosAddress,
                    );
                    ...
                    const PHMN_IBC =
                      "ibc/4698B7C533CB50F4120691368F71A0E7161DA26F58376262ADF3F44AAAA6EF9E";
                    phmnBal = await RpcService.getCosmosBalance(
                      "NEUTRON",
                      cosmosAddress,
                      PHMN_IBC,
                    );
                  ...
                  if (phmnFound && phmnBal >= 0) {
                    ...
                    if (!get().wallets.find((w) => w.id === phmnWallet.id)) {
                      set((state) => ({
                        wallets: [...state.wallets, phmnWallet],
                      }));
                    }
                  }
                  ...
                  if (chainConfig.id === "atomone-1") {
                    const photonBal = await RpcService.getCosmosBalance(
                      "ATOM_ONE",
                      cosmosAddress,
                      "uphoton",
                    );
                    if (photonBal > 0) {
                      ...
                      if (
                        !get().wallets.find((w) => w.id === photonWallet.id)
                      ) {
                        set((state) => ({
                          wallets: [...state.wallets, photonWallet],
                        }));
                      }
                    }
                  }
              ...
              try {
                  ...
                  for (const chainCfg of EVM_CHAINS) {
                    try {
                      ...
                      if (nativeBal > 0) {
                        ...
                        const currentWallets = get().wallets;
                        if (
                          !currentWallets.find((w) => w.id === newWallet.id)
                        ) {
                          set((state) => ({
                            wallets: [...state.wallets, newWallet],
                          }));
                      ...
                      for (const token of ERC20_TOKENS) {
                        ...
                        if (contract) {
                          const tokenBal = await RpcService.getErc20Balance(
                            chainCfg.rpc,
                            contract,
                            evmAddress,
                          );
                          if (tokenBal > 0) {
                            // Always show stablecoins
                            ...
                            if (
                              !get().wallets.find(
                                (w) => w.id === tokenWallet.id,
                              )
                            ) {
                              set((state) => ({
                                wallets: [...state.wallets, tokenWallet],
                              }));
                            }
                          }
                        }
                      ...
                      console.error(
                        `Failed Keplr discovery for ${chainCfg.name}:`,
                        err,
                      );
                    }
                  }
                ...
                console.warn(
                  "Keplr EVM connection failed or not accepted:",
                  err,
                );
              }
          ...
          // ---------------------------------------------------------
            ...
            if (chain === "EVM") {
              ...
              const currentWallets = get().wallets;
              const uniqueNewWallets = evmWallets.filter(
                (nw) => !currentWallets.find((w) => w.id === nw.id),
              );
            ...
            if (chain === "Solana" && address) {
              ...
              try {
                ...
                console.error(
                  "Failed to fetch Solana details, defaulting to 0:",
                  error,
                );
              }
              ...
              const cleanWallets = get().wallets.filter(
                (w) => !(w.address === address && w.chain === "Solana"),
              );
              ...
              for (const token of SOLANA_TOKENS) {
                try {
                  const tokenBal = await RpcService.getSplBalance(
                    address,
                    token.mint,
                  );
                  if (tokenBal > 0) {
                  ...
                  console.error(
                    `Failed to fetch ${token.symbol} on Solana:`,
                    e,
                  );
                }
              }
      ...
      sendTransaction: async (walletId, recipient, amount, memo) => {
        ...
        if (wallet.chain === "Cosmos") {
          ...
          else if (wallet.id.startsWith("phmn-juno"))
            chainId = "juno-1"; // PHMN CW20
          ...
          if (
            wallet.symbol === "PHMN" &&
            chainId === "juno-1" &&
            !wallet.id.includes("neutron") &&
            !wallet.id.includes("osmosis")
          ) {
        ...
        if (wallet.chain === "EVM") {
          ...
          if (wallet.name.toLowerCase().includes("rabby")) {
            ...
            web3Provider =
              (window as any).keplr?.ethereum || (window as any).ethereum;
          } else {
            // MetaMask or Generic
          ...
          if (currentChainIdHex !== targetChainId) {
            try {
              ...
              if (switchError.code === 4902) {
                ...
                throw new Error(
                  `Please add the ${chainCfg?.name || "correct"} network to your wallet.`,
                );
              }
              throw switchError;
            }
          ...
          try {
            ...
            if (wallet.symbol === "USDC" || wallet.symbol === "USDT") {
              const token = ERC20_TOKENS.find(
                (t) => t.symbol === wallet.symbol,
              );
              const contractAddr = (token?.contracts as any)[targetChainId];
              if (!contractAddr)
                throw new Error(
                  `Contract address not found for ${wallet.symbol} on this chain`,
                );
            ...
            if (
              ...
              throw new Error(
                `Insufficient ${wallet.symbol} for transaction + gas fees.`,
              );
            }
            throw err;
          }
      ...
      executeSkipMessages: async (messages: any[]) => {
        console.log("🚀 Starting executeSkipMessages", {
        ...
        for (let i = 0; i < messages.length; i++) {
          ...
          console.log(
            `\n📦 Processing message ${i + 1}/${messages.length} for chain: ${chainId}`,
            m,
          );
          ...
          const wallet = wallets.find(
            (w) => getChainForWallet(w)?.chain_id === chainId,
          );
          ...
          console.log(`✅ Found wallet for ${chainId}:`, {
          ...
          if (cosmosMsgWrapper) {
            ...
            const rpc = rpcKey
              ? (RPC_URLS as any)[rpcKey]
              : `https://${chainId.split("-")[0]}-rpc.publicnode.com`;
            ...
            if (typeof cosmosMsgWrapper.msg === "string") {
              ...
              try {
                ...
                throw new Error(
                  `Invalid message format from Skip Protocol: ${parseErr}`,
                );
              }
            } else {
            ...
            if (
              cosmosMsgWrapper.msg_type_url?.includes("wasm") ||
              cosmosMsgWrapper.msg_type_url?.includes("MsgExecuteContract")
            ) {
              console.log(
                `⚙️ Handling Wasm message conversion (msg -> Uint8Array)...`,
              );
              // For MsgExecuteContract, the 'msg' field is a JSON object in Skip response
              // but must be encoded as Uint8Array for CosmJS
              if (
                camelMsgObj.msg &&
                (typeof camelMsgObj.msg === "object" ||
                  typeof camelMsgObj.msg === "string")
              ) {
                const { toUtf8 } = await import("@cosmjs/encoding");
                const msgString =
                  typeof camelMsgObj.msg === "string"
                    ? camelMsgObj.msg
                    : JSON.stringify(camelMsgObj.msg);
            ...
            console.log(
              `✅ Final message object (camelCase):`,
              JSON.stringify(camelMsgObj, null, 2),
            );
            ...
            console.log(
              `📨 Encoded message for signing:`,
              JSON.stringify(encodeMsg, null, 2),
            );
            ...
            try {
              ...
              if (cosmosMsgWrapper.msg_type_url?.includes("cosmwasm")) {
                ...
                console.log(
                  `✍️ Calling signAndBroadcast with explicit fee... (Keplr should prompt now)`,
                );
          ...
          // ---------------------------------------------------------
            ...
            if (wallet.name.toLowerCase().includes("rabby"))
              web3Provider = (window as any).rabby || web3Provider;
            ...
            if (receipt) {
              console.log(
                `✅ EVM transaction confirmed! Hash: ${receipt.hash}`,
              );
              ...
              if (w.chain === "EVM") {
                ...
                if (chainCfg) {
                  if (w.symbol === "USDC" || w.symbol === "USDT") {
                    ...
                    if (contract) {
                      nativeBal = await RpcService.getErc20Balance(
                        chainCfg.rpc,
                        contract,
                        w.address,
                      );
                    ...
                    nativeBal = await RpcService.getBalance(
                      chainCfg.rpc,
                      w.address,
                    );
                  }
                ...
                if (w.symbol === "PHMN") {
                  if (w.name.includes("Juno")) {
                    const PHMN_CONTRACT =
                      "juno1rws84uz7969aaa7pej303udhlkt3j9ca0l3egpcae98jwak9quzq8szn2l";
                    nativeBal = await RpcService.getCw20Balance(
                      "JUNO",
                      PHMN_CONTRACT,
                      w.address,
                    );
                    ...
                    const PHMN_IBC =
                      "ibc/4698B7C533CB50F4120691368F71A0E7161DA26F58376262ADF3F44AAAA6EF9E";
                    nativeBal = await RpcService.getCosmosBalance(
                      "NEUTRON",
                      w.address,
                      PHMN_IBC,
                    );
                  ...
                  const chain = (
                    w.name.includes("Hub")
                      ? "COSMOS_HUB"
                      : w.name.includes("Juno")
                        ? "JUNO"
                        : w.name.includes("Neutron")
                          ? "NEUTRON"
                          : w.name.includes("Osmosis")
                            ? "OSMOSIS"
                            : w.name.includes("Atom One")
                              ? "ATOM_ONE"
                              : null
                  ) as any;
                  if (chain) {
