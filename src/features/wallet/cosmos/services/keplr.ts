...
export const KeplrService = {
  ...
  async connect(onWallet: (w: ConnectedWallet) => void): Promise<void> {
    ...
    for (const chainConfig of COSMOS_CHAINS) {
      try {
        ...
        console.log(
          `[Keplr] ${chainConfig.name} - Address: ${cosmosAddress}, Balance: ${nativeBal}, Price: ${price}`,
        );
        ...
        if (chainConfig.id === "juno-1") {
          ...
          try {
            const { SkipService } =
              await import("../../../trade/cosmos/services/skip");
            const osmoAssets = await SkipService.getAssets({
              chainId: "osmosis-1",
              includeCw20: true,
              includeEvm: false,
            });
            const usdcAssets = osmoAssets.filter((a: any) =>
              a.symbol.toUpperCase().includes("USDC"),
            );
            let totalUsdc = 0;
            for (const asset of usdcAssets) {
              const bal = await RpcService.getCosmosBalance(
                "OSMOSIS",
                cosmosAddress,
                asset.denom,
              );
            ...
            console.error("[Keplr] Error detecting Osmosis USDC", err);
          }
        }
    ...
    try {
      ...
      if (discovered) {
        ...
        (window.ethereum as any).isKeplr
      ) {
        keplrEvmProvider = window.ethereum;
      }

      if (keplrEvmProvider) {
        const accounts = await keplrEvmProvider.request({
        ...
        for (const chainCfg of EVM_CHAINS) {
          try {
            ...
            console.error(
              `[Keplr] Failed EVM discovery for ${chainCfg.name}:`,
              err,
            );
          }
        }
  ...
  // ----------------------------------------------------------------
  async executeSkipMessages(
    messages: any[],
    wallets: ConnectedWallet[],
    getChainForWallet: (w: ConnectedWallet) => any,
  ): Promise<string[]> {
    ...
    for (let i = 0; i < messages.length; i++) {
      ...
      if (cosmosMsgWrapper) {
        ...
        const offlineSigner = window.keplr.getOfflineSigner(chainId);
        const rpcUrl =
          COSMOS_RPC_URLS[chainId] || `https://${chainId}-rpc.publicnode.com`;
        ...
        const rawMsg =
          typeof cosmosMsgWrapper.msg === "string"
