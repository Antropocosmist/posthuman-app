/**
 * Rabby Wallet Provider — EVM
 * Dependencies: ethers
 *
 * Handles:
 *   - connect(): EVM multi-chain discovery via window.rabby
 *   - sendTransaction(): EVM native + ERC20 sends via window.rabby
 */
...
export const RabbyService = {

    // ----------------------------------------------------------------
    // connect() — EVM multi-chain via window.rabby
    // ----------------------------------------------------------------
    async connect(): Promise<ConnectedWallet[]> {
        const provider = (window as any).rabby || (window as any).ethereum
        if (!provider) {
            alert('Rabby Wallet not detected!')
            return []
        }
        ...
        for (const chainCfg of EVM_CHAINS) {
            try {
                ...
                if (nativeBal > 0) {
                    wallets.push({
                        id: `Rabby-${chainCfg.name}-${address.substr(-4)}`,
                        name: `${chainCfg.name} (Rabby)`,
                        chain: 'EVM',
                        address,
                        icon: `${BASE_URL}icons/rabby.png`,
                ...
                console.error(`[Rabby] Failed discovery for ${chainCfg.name}:`, e)
            }
        }
    ...
    // ----------------------------------------------------------------
    // sendTransaction() — EVM native + ERC20 via window.rabby
    // ----------------------------------------------------------------
    async sendTransaction(wallet: ConnectedWallet, recipient: string, amount: string): Promise<string> {
        const { ethers } = await import('ethers')

        const web3Provider = (window as any).rabby || (window as any).ethereum
        if (!web3Provider) throw new Error('Rabby provider not found')
        ...
        if (currentChainIdHex !== targetChainId) {
            try {
                ...
                if (switchError.code === 4902) throw new Error('Please add the correct network to Rabby.')
                throw switchError
            }
        }
